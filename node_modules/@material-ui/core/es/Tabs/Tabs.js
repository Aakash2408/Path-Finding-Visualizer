  ...
  const getTabsMeta = () => {
    ...
    if (tabsNode && value !== false) {
      ...
      if (children.length > 0) {
        ...
        if (process.env.NODE_ENV !== 'production') {
          if (!tab) {
            console.error([`Material-UI: the value provided \`${value}\` to the Tabs component is invalid.`, 'None of the Tabs children have this value.', valueToIndex.keys ? `You can provide one of the following values: ${Array.from(valueToIndex.keys()).join(', ')}.` : null].join('\n'));
          }
        }
