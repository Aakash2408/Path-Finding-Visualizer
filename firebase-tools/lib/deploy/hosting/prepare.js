...
module.exports = function (context, options) {
    ...
    _.each(context.hosting.deploys, function (deploy) {
        ...
        if (!cfg.public) {
            throw new FirebaseError('Must supply a public directory using "public" in each "hosting" config.');
        }
        ...
        if (cfg.i18n) {
            if (!cfg.i18n.root) {
                ...
                if (!fsutils.dirExistsSync(resolveProjectPath(options, i18nPath))) {
                    utils.logLabeledWarning("hosting", `Couldn't find specified i18n root directory ${clc.bold(cfg.i18n.root)} in public directory ${clc.bold(cfg.public)}.`);
                }
            }
        }
        const data = {
